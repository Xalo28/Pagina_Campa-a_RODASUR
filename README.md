...
> El frontend React vive en la carpeta `frontend/`. Tailwind se **compila** en el build (ya no usa CDN).
> Los colores de marca están en `frontend/tailwind.config.js`.

## 🚀 Cómo ejecutar

Requisitos: **Node.js 18+** (probado en Node 22).

El proyecto son **dos carpetas independientes**, cada una con su propio `package.json`:
`backend/` (API + servidor) y `frontend/` (React + Vite).

### Desarrollo (recarga en caliente) — recomendado
...
Abre **http://localhost:5173/**. Cada cambio en `frontend/src` se refleja al instante.
...
cd ../backend
npm install
npm start          # sirve el API + la app React ya compilada
```
...
> Para cambiarlas, edita `backend/config.js` o define las variables de entorno
> `ADMIN_USER` / `ADMIN_PASSWORD` **antes** de crear la base de datos
> (borra `backend/data/rodasur.db` si ya existe y quieres que se recreen).
...
│   ├── server.js           # Servidor Express (API + sirve ../frontend/dist)
...
> 📱 **Configura tu número de WhatsApp** en `frontend/src/lib/config.js` (constante `WHATSAPP_NUMBER`,
...
Los datos de asesores, FAQs y la dirección/horario están en `frontend/src/lib/asesores.js`
...
- Respaldo manual en cualquier momento (desde `backend/`): `npm run backup`.

## 🚀 Despliegue a un servidor (resumen)

1. Instala Node 18+ en el servidor y sube el proyecto.
2. Crea el archivo `backend/.env` con tus secretos (y `SMTP_*` si usarás correos).
3. Compila el frontend: `cd frontend && npm install && npm run build`.
4. Instala el backend: `cd ../backend && npm install`.
5. Ejecútalo con un gestor de procesos, p. ej. **PM2**: `pm2 start server.js --name rodasur --cwd backend`.
6. Pon un **nginx** por delante como *reverse proxy* al puerto 3000 y activa **HTTPS**
   (Let's Encrypt / Certbot). Si usas proxy, añade `app.set('trust proxy', 1)` en `server.js`.

- La base de datos SQLite vive en `backend/data/rodasur.db`; las imágenes subidas en `backend/public/uploads/`.
